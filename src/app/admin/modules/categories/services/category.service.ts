...
export class CategoryService {
  ...
  addCategory(data) {
    return this._http.post<Category>(`${environment.API}category`, data).pipe(
      take(1)
    )
  }

  deleteCategory(id) {
    return this._http.delete<Category>(`${environment.API}category/${id}`).pipe(
      take(1)
    )
  }

  updateCategory(id, data){
    return this._http.put<Category>(`${environment.API}category/${id}`, data).pipe(
      take(1)
    )
  }
  //
