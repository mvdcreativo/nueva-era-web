...
export class ProductService {
  ...
  private products(): Observable<Product[]> {
    return this.httpClient.get<Product[]>(`${environment.API}product`);
  }
  ...
  public getProduct(slug: any) {
    ...
    return this.httpClient.get<Product>(`${environment.API}product-by-slug/${slug}`).pipe(
      take(1)
    )
  ...
  public searchProduct(data: any) {
    ...
    return this.httpClient.get<Product[]>(`${environment.API}search`, data).pipe(
      take(1)
    )
    // return this.products.find(product=> product.id === id);
...
*/
  ...
  public addToCompare(product: Product): Product | boolean {
    ...
    if (this.hasProduct(product)) {
      ...
      this.snackBar.open('El producto  ' + product.name + ' already added to comparison list.', '×', { panelClass: 'error', verticalPosition: 'top', duration: 3000 });

    } else {
      if (products.length < 4)
        products.push(product);
      message = 'El producto ' + product.name + ' has been added to comparison list.';
      status = 'success';
  ...
  public getProductByBrand(brand){
    return this.httpClient.get<Product[]>(`${environment.API}brand-by-slug/${brand}`)
  }
